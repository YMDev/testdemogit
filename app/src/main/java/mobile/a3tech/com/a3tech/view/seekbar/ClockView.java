...
package mobile.a3tech.com.a3tech.view.seekbar;
...
import mobile.a3tech.com.a3tech.utils.Utils;
...
import  mobile.a3tech.com.a3tech.utils.Utils.FontType.*;
import  mobile.a3tech.com.a3tech.utils.Utils.*;
...
public class ClockView extends LinearLayout {
    ...
    public ClockView(Context context, AttributeSet attrs) {
        ...
        if (!isInEditMode()) {
            ...
            mTimeMeridianText.setTypeface(Utils.getRobotoTypeface(context, Utils.FontType.REGULAR));
    ...
    private void setClockText(DateTime newCurrentTime) {
        if (mIs24HourFormat) {
            mTimeText.setText(Utils.SIMPLE_DATE_FORMAT_HOURS.print(newCurrentTime));
            mTimeMeridianText.setText(R.string.flights_app_short_hrs_cbd);
        } else {
            mTimeText.setText(Utils.SIMPLE_DATE_FORMAT_AM_PM.print(newCurrentTime));
            mTimeMeridianText.setText(Utils.SIMPLE_DATE_FORMAT_MERIDIAN.print(newCurrentTime).toLowerCase(Locale.US));
        }
    ...
    private void setSeekBarStatus(DateTime newCurrentTime) {
        if (mValidTimeInterval.contains(newCurrentTime)) {
            if (newCurrentTime.getDayOfWeek() == mValidTimeInterval.getStart().getDayOfWeek()) {
                mCircularClockSeekBar.setSeekBarStatus(CircularClockSeekBar.ClockRangeStatus.VALID_RANGE);
                mTimeWeekDayText.setVisibility(GONE);
            } else {
                mCircularClockSeekBar.setSeekBarStatus(CircularClockSeekBar.ClockRangeStatus.DIFFERENT_DAY_OF_WEEK);
                mTimeWeekDayText.setVisibility(VISIBLE);
                mTimeWeekDayText.setText(newCurrentTime.toString("EEE"));
            }
        } else {
            mCircularClockSeekBar.setSeekBarStatus(CircularClockSeekBar.ClockRangeStatus.INVALID_RANGE);
            mTimeWeekDayText.setVisibility(GONE);
        }
    }
