...
public class ServicesARendreFragment extends Fragment implements DataLoadCallback {
	...
	public static ServicesARendreFragment newInstance() {
		ServicesARendreFragment fragment = new ServicesARendreFragment();
        return fragment;
    }	
	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.services_a_rendre_fragment, container, false);
	...
	private void initView() {
		mAdapter = new ServicesAdapter(getActivity(), mList,true);
		...
		mListView.setOnRefreshListener(new CustomListView.OnRefreshListener() {
			...
			public void onRefresh() {
				start = 0;
				MissionManager.getInstance().missionsSuivies(Constant.REFRESH_DATA_FINISH, connectedUser,String.valueOf(start), String.valueOf(limit),password,ServicesARendreFragment.this);
			}
		});
		mListView.setOnLoadListener(new CustomListView.OnLoadMoreListener() {
			@Override
			public void onLoadMore() {
				MissionManager.getInstance().missionsSuivies(Constant.LOAD_DATA_FINISH,connectedUser,String.valueOf(start), String.valueOf(limit),password,ServicesARendreFragment.this);
		...
		mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			public void onItemClick(AdapterView<?> paramAnonymousAdapterView,
				 ...
				 Mission mission = (Mission) ServicesARendreFragment.this.mAdapter.getMissions().get(position - 1);
		            if (Integer.parseInt(mission.getStatut()) != 5) {
		                Intent mainIntent = new Intent(ServicesARendreFragment.this.getActivity(), DetailServiceViewPagerFragment.class);
		                ...
		                ServicesARendreFragment.this.getActivity().startActivityForResult(mainIntent, 101);
		            }
			}
		});
